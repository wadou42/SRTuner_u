...
if __name__ == "__main__":
    ...
    build_home_flag = os.path.join(benchmark_home, f"polybench-instance{2*idx}", "stencils/jacobi-1d")
    build_home_base = os.path.join(benchmark_home, f"polybench-instance{2*idx+1}", "stencils/jacobi-1d")
    ...
    POLYBENCH_HOME_FLAG = f"/home/whq/dataset/polybench/polybench-instance{2*idx}"
    POLYBENCH_HOME_BASE = f"/home/whq/dataset/polybench/polybench-instance{2*idx+1}"
    ...
    result_file = os.path.join("tune_result_polybench/jacobi-1d", f"{idx}_result.json")
    ...
    for tuner in [RandomTuner(search_space, evaluator, default_setting)]:
        ...
        if best_opt_setting is not None:
            ...
            line = f"Tuning jacobi-1d w/ {tuner.name}: {default_perf:.3f}/{best_perf:.3f} = {default_perf/best_perf:.3f}x"
            print(line)
            with open(f"tune_result/jacobi-1d_result.txt", "a") as ofp:
                ofp.write(line + "\n")
