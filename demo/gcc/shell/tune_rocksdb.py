...
from manager.rocksdb_namager import RocksDBManager
...
def convert_to_str(opt_setting, search_space):
    ...
    opt_setting = c.solve(opt_config=opt_setting)
    ...
    if "-fno-toplevel-reorder" in str_opt_setting:
            str_opt_setting = str_opt_setting.replace("-fsection-anchors", "-fno-section-anchors")
    # print(len(str_opt_setting.split()), flush=True)
    return str_opt_setting


# Define tuning task
class RocksDBEvaluator(Evaluator):
    def __init__(self, path, num_repeats, search_space, manager, manager_base=None, enable_gperftools=False, artifact="a.out", result_file="rocksdb_data.json"):
    ...
    #         with open ("opts_list/rocksdb_bad_config.txt", "a") as ofp:
    #             ofp.write(f"{str_opt_setting}\n")
    #         return -1
    #     return 0
    
    def build(self, str_opt_setting):
        info = self.manager.build(" -g" + str_opt_setting, jobs=32)
        if info == -1:
            with open ("opts_list/rocksdb_bad_config.txt", "a") as ofp:
                ofp.write(f"{str_opt_setting}\n")
            return -1
        self.manager_base.build(" -g -O3", jobs=32)
        return 0
    ...
    def evaluate(self, opt_setting, num_repeats=-1):
        ...
        print(f"Evaluating with flags: {flags}", flush=True)
        ...
        report_infos = RocksDBManager.analysis_report(self.manager, self.manager_base)
...
if __name__ == "__main__":
    ...
    with open("tuning_rocksdb_Ofast_result.txt", "w") as ofp:
        ofp.write("=== Result ===\n")
    ...
    evaluator = RocksDBEvaluator(path=None, num_repeats=1, manager=manager, manager_base=manager_base, search_space=search_space)
