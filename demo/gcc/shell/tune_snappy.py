...
from manager.snappy_manager import SnappyManager
...
def convert_to_str(opt_setting, search_space):
    c = ConstrainsSolver(constrains_file="constrains/zstd.txt")
    c.solve(opt_config=opt_setting)

    # str_opt_setting = " -O" + str(opt_setting["stdOptLv"])
    str_opt_setting = " -g -O3"
...
# Define tuning task
class SnappyEvaluator(Evaluator):
    def __init__(
        self,
        path,
        num_repeats,
    ...
    def build(self, str_opt_setting): #type: ignore
        print(f"Building with flags: {str_opt_setting}")
...
if __name__ == "__main__":
    budget = 50
    gcc_optimization_info = "opts_list/gcc_opts.txt"
    ...
    cpu_cores = 120
    build_dir = "/home/whq/dataset/snappy/snappy-instance2"

    lzbench_home = "/home/whq/dataset/lzbench"
    num_repeat = 1
    enable_gperftools = True

    manager = SnappyManager(
        build_dir=build_dir,
        cpu_cores=cpu_cores,
        enable_gperftools=enable_gperftools,
        lzbench_home=lzbench_home,
        num_repeat=1
    )

    manager_base = None

    evaluator = SnappyEvaluator(
        path=None,
        num_repeats=1,
        manager=manager,
    ...
    tuners = [SRTuner(search_space, evaluator, default_setting)]
    ...
    for tuner in tuners:
        ...
        if best_opt_setting is not None:
            ...
            print(
                f"Tuning ZSTD w/ {tuner.name}: {default_perf:.3f}/{best_perf:.3f} = {default_perf/best_perf:.3f}x"
            )
            with open(f"tune_result/ZSTD_result.txt", "a") as ofp:
                ofp.write(
                    f"Tuning ZSTD w/ {tuner.name}: {default_perf:.3f}/{best_perf:.3f} = {default_perf/best_perf:.3f}x\n"
                )
